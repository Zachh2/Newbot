...
module.exports.run = async ({ event, api, args, Users }) => {
  ...
  let dataAll = Object.keys(data).map(uid => ({
    userID: uid,
    exp: data[uid].exp || 0
  }));
  ...
  const getRank = (uid) =>
    dataAll.findIndex(item => item.userID == uid) + 1;

  // SELF
  if (args.length == 0) {
    const uid = event.senderID;

    if (!data[uid]) data[uid] = { exp: 0, level: 1 };

    const rank = getRank(uid);
    const name =
      global.data.userName.get(uid) || (await Users.getNameUser(uid));

    const point = await this.getInfo(uid);
